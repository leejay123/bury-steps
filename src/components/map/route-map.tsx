 ...
 * page by default. Nothing is fetched beyond map tiles, and those load as
 * plain <img> tags via Leaflet, not fetch()/XHR — no CSP connect-src entry
 * needed, no external API key, nothing that can be silently blocked by a
 * network in between the way the fetch-based 3D view (route-map-view-impl
 * .tsx, opened via the "View in 3D" button below this) can be. This is the
 * one guaranteed-to-work view; 3D is a bonus on top of it, not a
 * replacement for it — brought back after MapLibre's fetch-based tile
 * loading proved to hang on at least one real device/network, which would
 * otherwise have taken the *only* route map down with it.
 */
