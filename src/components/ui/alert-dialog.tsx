...
import { OverlayRootContext, restorePagePointerEvents } from "@/components/overlay-root";
...
function AlertDialog({
  ...
  return (
    <AlertDialogPrimitive.Root
      ...
      onOpenChange={(open) => {
        if (!open) restorePagePointerEvents();
        onOpenChange?.(open);
      }}
      {...props}
    />
...
function AlertDialogOverlay({ className, ...props }: React.ComponentProps<typeof AlertDialogPrimitive.Overlay>) {
  return (
    <AlertDialogPrimitive.Overlay
      ...
      className={cn(
        "data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:!pointer-events-none data-[state=open]:fade-in-0 fixed inset-0 z-[70] bg-black/30 backdrop-blur-sm",
        className,
      )}
      {...props}
    />
