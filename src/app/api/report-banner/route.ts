...
export async function GET() {
  const setting = await prisma.siteSetting.findUnique({
    ...
    select: { reportBannerData: true },
  });

  if (!setting?.reportBannerData || setting.reportBannerData.length === 0) {
  ...
  const sniffed = sniffImageMime(setting.reportBannerData);
  if (!sniffed) {
    return new NextResponse("Not found", { status: 404 });
  }

  return new NextResponse(Buffer.from(setting.reportBannerData), {
    headers: {
