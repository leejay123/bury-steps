...
function CalendarDropdown({ disabled, onChange, options, value }: DropdownProps) {
  return (
    <Select
      disabled={disabled}
      onValueChange={(next) => {
...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      className={cn(
        "group/calendar bg-background p-3 [--cell-size:--spacing(8)] pointer-coarse:[--cell-size:min(2.75rem,calc((100vw_-_3.5rem)/7))] [[data-slot=card-content]_&]:bg-transparent [[data-slot=popover-content]_&]:bg-transparent",
